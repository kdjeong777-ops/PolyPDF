...
Write-Host ""
Write-Host "업로드(1회): gh release create components `"$ff`" `"$zip`" --title `"Components (ffmpeg/Tesseract)`" --prerelease --notes `"녹화·OCR 구성요소`""
...
Write-Host "  (SOT 마스터 §14.5 U16: --prerelease 필수 — 없으면 유일한 정식 릴리스라 'Latest' 가 된다)"
