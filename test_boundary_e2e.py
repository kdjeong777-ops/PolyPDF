...
def wheel_cross(view, dy):
    """쪽·파일 경계를 **넘기려는** 손짓 — 휠 **한 칸**.

    260914-1(입력 SOT §2.2 규칙 2, 사용자 지시): 쪽 맞춤이면 한 칸에 한 쪽이다(종전
    260910-5 의 '첫 칸 버림 + 350ms' 는 개정). 파일 경계에서는 넘어가기 전에 묻는다(§2.4).
    앞 손짓 직후의 잠금(규칙 5·6)은 사람이 손을 떼고 다시 굴린 것으로 치고 비운다 —
    실제로 기다리는 대신 넘긴 시각을 과거로 돌린다.
    """
    try:
        view._flip_ms = -1e9
        view._last_wheel_ms = -1e9
    except AttributeError:
        pass
    wheel(view, dy)                       # 이 한 칸에서 넘어간다
