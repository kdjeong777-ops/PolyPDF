...
sd = SettingsDialog({"presentation_overlap_pct": 12,   # 260628(B5): presentation_split 설정 제거
                     "presentation_dual": True})        # 260905: presentation_topbar_h 삭제
...
chk("presentation_overlap_pct" not in rp
    and "presentation_dual" not in rp, "result_prefs 에 발표 보기 키 없음")
...
mw._prefs["presentation_dual"] = True          # 260905(§4.1): 좌우 2쪽·맞쪽은 영속
mw._prefs["presentation_facing"] = True
mw._apply_prefs(sd.result_prefs())     # 발표 키 미포함 → 보존되어야
chk(mw._prefs["presentation_overlap_pct"] == 25
    and mw._prefs["presentation_dual"] is True
    and mw._prefs["presentation_facing"] is True
    , "발표 보기 설정값 보존",
    str([mw._prefs[k] for k in ("presentation_overlap_pct","presentation_dual","presentation_facing")]))
chk("presentation_topbar_h" not in mw._prefs, "상단 띠 높이 설정 제거(§6)")
...
chk(not hasattr(pres, "set_topbar_height"), "set_topbar_height API 제거(§6)")
chk(not hasattr(pres, "cropSettingsRequested"), "죽은 cropSettingsRequested 시그널 제거(§2.1)")
