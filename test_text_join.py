...
try:
    ...
    # 260916-1(SOT §3.7.10): 한글끼리 이을 때 빈칸을 넣을지는 **말뭉치가 정한다**.
    #   여기 글은 어절 경계가 분명하므로 빈칸이 들어간다(종전에는 무조건 붙였다).
    chk(r[0]["text"] == "첫째 줄이길게이어지고 둘째 줄로이어진다.",
        "⑦ 한글끼리 — 어절 경계면 띄운다", repr(r[0]["text"]))
    # ★ 붙여야 하는 두 자리는 그대로 붙어야 한다 — §3.7.3·§3.7.7 이 실측으로 적어 둔 것.
    for a, b, want, tag in (
            ("성격에따라취사", "선택되어야 한다.", "성격에따라취사선택되어야 한다.", "따라취사+선택되어야"),
            ("건설공사의무사", "용대상 자재이다.", "건설공사의무사용대상 자재이다.", "의무사+용대상"),
    ):
        r2 = tx.join_sentences(para([(a, M), (b, 300.0)]))
        chk(len(r2) == 1 and r2[0]["text"] == want,
            "⑦ 낱말 가운데서 잘린 것은 붙인다 — %s" % tag,
            repr(r2[0]["text"] if r2 else None))
    # kiwi 를 못 쓰면 기하 규칙(§3.7.7)으로 돌아간다 — 붙임이 기본이다.
    _keep = tx._KIWI["bad"]
    tx._KIWI["bad"] = True
    try:
        r3 = tx.join_sentences(para([("첫째 줄이길게이어지고", M), ("둘째 줄로이어진다.", 300.0)]))
        chk(r3[0]["text"] == "첫째 줄이길게이어지고둘째 줄로이어진다.",
            "⑦ kiwi 가 없으면 종전 기하 규칙으로 돌아간다", repr(r3[0]["text"]))
    finally:
        tx._KIWI["bad"] = _keep
