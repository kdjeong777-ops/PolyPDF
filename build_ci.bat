...
REM 260618-12: ffmpeg(녹화)/Tesseract(OCR)는 배포본에 동봉하지 않음(용량↓).
REM   앱 '도구 → 구성요소 설치(녹화·OCR)'가 릴리스 components 태그에서 설치 폴더로 받음.
REM   (되돌리려면 아래 두 줄의 set 을 if exist ... 동봉 형태로 복원)
set TESS_ARG=
set FFMPEG_ARG=
set NLTK_ARG=
