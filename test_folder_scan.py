...
검사 대상(응답성 SOT §4.1 '폴더 열기 3단 규칙' / '시작 복원은 창을 띄운 뒤'):
...
try:
    ...
    # 260907-1: **앞 단계의 검사 워커가 끝나기를 먼저 기다린다.** `_probe_tick` 은 워커가
    #   살아 있으면 큐를 그대로 두고 돌아가므로(정상 동작), 안 기다리면 아래 ④-b
    #   '넘긴 큐는 비워진다' 가 스위트 부하에 따라 들쭉날쭉 실패한다(실측 재현).
    _t_end = time.time() + 30
    while time.time() < _t_end and bt3._probe_worker is not None:
